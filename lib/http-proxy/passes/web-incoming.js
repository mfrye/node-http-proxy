  ...
  function stream(req, res, options, _, server, clb) {
    ...
    proxyReq.on('response', function(proxyRes) {
      ...
      if (!proxyRes.aborted) {
        ...
        var isHtml = contentType ? contentType.match( 'text/html' ) : false;

        if (contentEncoding == 'gzip' && isHtml) {
          proxyRes.pipe(zlib.createGunzip()).pipe(res);
        }
        else if (contentEncoding == 'deflate' && isHtml) {
          proxyRes.pipe(inflate.createInflate()).pipe(res);
        }

        else if (contentEncoding == 'br' && isHtml) {
          proxyRes.pipe(brotli.decompressStream()).pipe(res);
        }
